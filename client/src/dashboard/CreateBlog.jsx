...
function CreateBlog() {
  ...
  const handleSubmitBlogData = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:3000/api/blog/create", {  title, about, category, blogImg: photoToSend },
     ...
     <div className='min-h-screen flex justify-center items-center bg-gray-100 mx-auto'>
        <div className='w-full max-w-md bg-blue-200 shadow-md rounded p-8'>
          <form onSubmit={handleSubmitBlogData}>
            ...
            <h1 className='text-xl font-semibold mb-6'>Create Blog</h1>
            <div className='mb-4'>
            ...
            <button type="submit" className='w-full p-2 bg-blue-500 hover:bg-blue-800 duration-300 rounded-md text-white'>CREATE</button>
          </form>
        </div>
      </div>
    </>
